...
# not package-qualified), so that directory must be on sys.path before
# importing build_vector_store — matching how vector_db.py/retrieve.py
# bootstrap themselves.
_VECTOR_STORE_DIR = Path(__file__).resolve().parent.parent / "rag" / "vector_store"
if str(_VECTOR_STORE_DIR) not in sys.path:
    sys.path.insert(0, str(_VECTOR_STORE_DIR))


def _reindex_rag_store() -> Dict[str, Any]:
    """Rebuild the Chroma collection from whatever is on disk in
    resources/ right now. Called after every add/remove so an admin's
    change is live for the *next* query, not just written to disk —
    per the brief: 'confirm next query actually reflects the change,
    not just uploaded.' Reindexing here (synchronously, on write) is
    the simplest correct choice for this corpus size; if it ever
    becomes too slow to do inline, move it to a background task, but
    don't drop the guarantee that the caller can rely on it having
    happened by the time the request returns."""
    from vector_db import build_vector_store  # local import: needs sys.path above

    collection = build_vector_store()
    return {"reindexed": True, "chunk_count": collection.count()}
...
def upload_rag_doc(payload: RAGDocUpload) -> Dict[str, Any]:
    if not payload.name.endswith(".md"):
        raise HTTPException(status_code=400, detail="RAG documents must be .md files")
    target = RAG_RESOURCES_DIR / payload.name
    target.write_text(payload.content, encoding="utf-8")
    try:
        reindex_result = _reindex_rag_store()
    except Exception as exc:
        # The file is on disk but the index couldn't be rebuilt — surface
        # this as a real error rather than a silent 200, since the whole
        # point of reindexing here is that a caller can trust a 200 means
        # the next query will actually see the new doc.
        raise HTTPException(
            status_code=500,
            detail=f"Document written but reindex failed: {exc}",
        ) from exc
    return {"status": "ok", "name": payload.name, "path": str(target), **reindex_result}
...
def delete_rag_doc(doc_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Document deleted but reindex failed: {exc}",
        ) from exc
    return {"status": "ok", "deleted": doc_name, **reindex_result}
